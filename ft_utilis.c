...
void	mandel_vs_julia(int x, int y, t_fractal *fractol)
{
	if (!ft_strncmp(fractol->name, "mandelbrot", 10))
	{
		...
		fractol -> c.real_x = scale(x, -2, 2, WIDTH) * fractol -> zomm_in
			+ fractol -> a;
		fractol -> c.imaginary_y = scale(y, 2, -2, HEIGHT) * fractol->zomm_in
			+ fractol->b;
	}
	else
	{
		fractol -> z.real_x = scale(x, -2, 2, WIDTH) * fractol -> zomm_in
			+ fractol -> a;
		fractol -> z.imaginary_y = scale(y, 2, -2, HEIGHT) * fractol -> zomm_in
			+ fractol -> b;
